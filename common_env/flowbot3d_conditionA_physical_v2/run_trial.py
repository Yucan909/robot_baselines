...
# ============================================================
# Senior unified bottom logic
# ============================================================

SENIOR_ARM_STIFFNESS = 1800.0
SENIOR_ARM_DAMPING = 360.0

SENIOR_FINGER_STIFFNESS = 8000.0
SENIOR_FINGER_DAMPING = 1600.0

SENIOR_CONTACT_STATIC_FRICTION = 2.0
SENIOR_CONTACT_DYNAMIC_FRICTION = 2.0
SENIOR_CONTACT_RESTITUTION = 0.0

# OPEN / TO_GRASP / HOLD / OPERATE:
# nearly-free articulated target.
SENIOR_OPERATE_TARGET_STIFFNESS = 0.0
SENIOR_OPERATE_TARGET_DAMPING = 0.05

# CLOSE:
# stabilize prescribed initial articulation.
SENIOR_CLOSE_TARGET_STIFFNESS = 5000.0
SENIOR_CLOSE_TARGET_DAMPING = 500.0
...
def execute_trial(
    ...
    metrics = {
        ...
        "bottom_logic":
            {
                "version":
                    "senior_bottom_20260901",

                "condition":
                    "A_real_physical_twofinger",

                "robot_geometry":
                    "full_panda_two_finger",

                "interaction":
                    "finger_collision_contact_friction_only",

                "ideal_suction":
                    False,

                "artificial_hand_object_drive":
                    False,

                "finger_object_weld":
                    False,

                "distance_based_grasp_gate":
                    False,
                ...
                "operate_target_drive":
                    [0.0, 0.05],

                "physical_grasp_settle_steps":
                    GRASP_SETTLE_STEPS,

                "physical_grasp_tail_steps":
                    GRASP_TAIL_STEPS,

                "physical_grasp_min_bilateral_fraction":
    ...
    try:
        ...
        panda = PandaTwoFingerController(
            ...
            # Senior unified canonical contact material.
            finger_static_friction=(
                SENIOR_CONTACT_STATIC_FRICTION
            ),

            finger_dynamic_friction=(
                SENIOR_CONTACT_DYNAMIC_FRICTION
            ),

            finger_restitution=(
                SENIOR_CONTACT_RESTITUTION
            ),
        )
        ...
        # ----------------------------------------------------
        # Shared senior contact material.
        #
        # Fingers already use panda.finger_material.
        # Apply the same material to the selected target link.
        #
        # IMPORTANT:
        # this is ordinary collision/friction material,
        # NOT a suction/weld constraint.
        # ----------------------------------------------------
        ...
        for collision_shape in (
            ...
            collision_shape.set_physical_material(
                panda.finger_material
            )

        metrics[
            "physical_contact_material"
        ] = {
            "static_friction":
                SENIOR_CONTACT_STATIC_FRICTION,
        ...
        # ----------------------------------------------------
        # Senior phase:
        # INIT -> PREGRASP keeps target at prescribed q.
        # ----------------------------------------------------

        target_joint.set_drive_property(
            stiffness=SENIOR_CLOSE_TARGET_STIFFNESS,
            damping=SENIOR_CLOSE_TARGET_DAMPING,
        )
        ...
        # Initial benchmark qpos already has fingers open.
        # Keep the open actuator command active.
        panda.open_gripper()
        ...
        # ----------------------------------------------------
        # Senior phase:
        # PHASE_GRIPPER_OPEN + PHASE_TO_GRASP
        # target articulation is free.
        # ----------------------------------------------------

        target_joint.set_drive_property(
            stiffness=SENIOR_OPERATE_TARGET_STIFFNESS,
            damping=SENIOR_OPERATE_TARGET_DAMPING,
        )
        ...
        metrics[
            "progress_drift_after_free_approach"
        ] = float(
        ...
        # ----------------------------------------------------
        # REAL TWO-FINGER CLOSE
        #
        # Senior CLOSE phase re-stabilizes the target joint.
        #
        # During this entire close window:
        #   - target qpos is pinned to prescribed initial q
        #   - target qvel is zeroed
        #   - fingers physically move using their real joints
        #
        # There is still NO hand-object / finger-object drive.
        # ----------------------------------------------------

        object_qpos_close = np.asarray(
            obj.get_qpos(),
            dtype=np.float64,
        ).copy()
        ...
        metrics[
            "progress_after_locked_close"
        ] = float(
            get_progress()
        )

        # ----------------------------------------------------
        # HOLD / grasp validation:
        #
        # release target to actual operate physics FIRST.
        #
        # Now physical bilateral contact must hold the target
        # without suction or weld.
        # ----------------------------------------------------
        ...
        metrics[
            "progress_drift_after_physical_grasp"
        ] = float(
            pre_pull_progress
            - initial_ratio
        )
        ...
        # ----------------------------------------------------
        # Formal physical operation.
        #
        # No suction.
        # No object drive.
        # Force transmission can only come from:
        #   collision + normal force + friction.
        # ----------------------------------------------------
